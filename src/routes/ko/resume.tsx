                  ...
                  details: [
                    {
                      label: "구현",
                      content:
                        "z-score, IQR, CV 기반 모델 성능 지표 산출 및 근거 SQL 쿼리 노출 기능 개발.",
                    },
                  ],
                },
              ]}
              ...
              projects={[
                {
                  ...
                  description:
                    "블록체인 저장소의 비용/성능 한계를 극복하는 대규모 오프체인 스토리지 솔루션",
                  techStack: [
                    "Go",
                    "MongoDB",
                    "Redis Streams",
                  ...
                  details: [
                    {
                      label: "고성능 스토리지 아키텍처",
                      content:
                        "Celestia/Ceph 연구 기반 Reed-Solomon Erasure Coding 적용으로 데이터 복구 및 저장 효율 최적화.",
                    },
                    {
                      label: "Custom Merkle Patricia Trie",
                      content:
                        "go-ethereum 기반 In-memory Trie 구현. 배치 감사 로그의 무결성을 단일 해시로 증명.",
                    },
                    {
                      label: "Redis Streams 파이프라인",
                      content:
                        "At-least-once Delivery 및 Dead Letter Queue로 안정적인 비동기 처리 구현.",
                    },
                    {
                      label: "API & Testing",
                      content:
                        "gRPC-Gateway 듀얼 프로토콜 지원. Ginkgo + Locust 기반 E2E 및 부하 테스트 자동화.",
                    },
                  ],
                  outcome:
                    "단일 인스턴스 1,500 TPS, 데이터 무결성 99.999% 달성. 장애 시 고가용성 확보.",
                },
                {
                  title: "Enterprise Blockchain Indexer SDK",
                  description:
                    "ETL 파이프라인 기반의 고성능 커스텀 인덱서 구축을 위한 Rust SDK 개발",
                  techStack: ["Rust", "gRPC", "PostgreSQL", "Diesel", "Tokio"],
                  details: [
                    {
                      label: "ETL 아키텍처",
                      content:
                        "gRPC 스트리밍(Extract), BCS 이벤트 디코딩(Transform), 청크 단위 배치 적재(Load)를 처리하는 제네릭 ETL 엔진 설계.",
                    },
                    {
                      label: "이중 테이블 전략",
                      content:
                        "O(1) 상태 조회와 이력 추적을 동시에 지원하는 History/Snapshot 테이블 패턴 구현.",
                    },
                    {
                      label: "Rust SDK 추상화",
                      content:
                        "복잡한 체인 로직을 Trait으로 추상화하여 내부 서비스 개발자들의 보일러플레이트 코드 90% 감소.",
                    },
                  ],
                  outcome:
                    "다수 엔터프라이즈 블록체인 서비스의 인덱싱 패턴 표준화 및 데이터 통합 가속화.",
                },
              ]}
            />
              ...
              projects={[
                {
                  ...
                  description:
                    "초기 설계부터 배포까지 MVP 개발 총괄 (스핀오프 창업 참여)",
                  techStack: [
                    "Node.js (NestJS)",
                    "PostgreSQL (PostGIS)",
                    "Turf.js",
                  ...
                  details: [
                    {
                      label: "공간 데이터 처리",
                      content:
                        "PostGIS + Turf.js 기반 가상 토지 폴리곤 연산 및 R-tree 인덱싱 최적화.",
                    },
                    {
                      label: "NestJS 마이크로서비스",
                      content:
                        "모듈러 아키텍처 설계 및 블록체인 연동 독립 서비스 구축.",
                    },
                    {
                      label: "결제 & 보안",
                      content:
                        "PayPal/Coocon 연동, JWT + OAuth 2.0 기반 인증 시스템.",
                    },
                  ],
                  ...
                  details: [
                    {
                      label: "기능 구현",
                      content:
                        "작품 아카이브, 전시 안내, 뉴스레터 및 비개발자용 관리자 페이지 개발.",
                    },
                    {
                      label: "인프라",
                      content:
                        "AWS EC2, S3, Route53 기반 서버 및 스토리지 구성.",
                    },
                  ],
                  outcome:
                    "현재까지 학과 공식 커뮤니케이션 허브로 안정적 운영 중.",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:grid-cols-2 print:gap-4">
            <Card className="print:shadow-none">
              ...
              <CardContent className="space-y-2 text-sm text-muted-foreground print:text-xs">
                <p>
                  • <strong>Concurrency Control:</strong> Go, Rust 기반 동시성
                  제어 및 최적화.
                </p>
                <p>
                  • <strong>Distributed Lock:</strong> Redis Redlock을 활용한
                  분산 락 적용.
                </p>
                <p>
                  • <strong>Integrity:</strong> 금융/블록체인 시스템 수준의
                  데이터 정합성 보장 전략 수립.
                </p>
              </CardContent>
            </Card>
            <Card className="print:shadow-none">
              ...
              <CardContent className="space-y-2 text-sm text-muted-foreground print:text-xs">
                <p>
                  • <strong>Kubernetes (EKS):</strong> MSA 아키텍처 구축 및 운영
                  경험.
                </p>
                <p>
                  • <strong>IaC & Helm:</strong> Terraform 및 Helm Charts를 통한
                  인프라/배포 관리.
                </p>
                <p>
                  • <strong>Observability:</strong> Istio Service Mesh,
